...
\subsubsection{Motivation}
The Python programming language is commonly used in scientific computation applications because of its natural form of expression,
large collection of open-source packages and its ease of use when compared with traditional languages like C. However, as an interpreted language
Python is not suited for direct use in HPC applications, and currently available methods for reducing the interpreter overhead introduce
additional complexity and are still not as performant as straight C code.
