        ...
        for(int xx=0;xx<=13;xx++) {
            for(int yy=0;yy<=13;yy++) {
                for(int zz=0;zz<=13;zz++) {
                    for(int time=1;time<=time_size-2;time++) {
                        int skew=4*time;
                        for(int x=max(4*time+4,20*xx+4*time);x<=min(4*time+271,20*xx+4*time+19);x++) {
                            for(int y=max(4*time+4,20*yy+4*time);y<=min(4*time+271,20*yy+4*time+19);y++) {
                                #pragma ivdep
                                #pragma omp simd
                                for(int z=max(4*time+4,20*zz+4*time);z<=min(4*time+271,20*zz+4*time+19);z++) {
