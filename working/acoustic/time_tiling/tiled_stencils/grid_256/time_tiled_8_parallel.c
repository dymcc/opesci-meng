    ...
    #pragma omp parallel
    {
        ...
        for(int xx=0;xx<=33;xx++) {
            for(int yy=0;yy<=33;yy++) {
                for(int zz=0;zz<=33;zz++) {
                    for(int time=1;time<=time_size-2;time++) {
                        int skew=4*time;
                        for(int x=max(4*time+4,8*xx+4*time);x<=8*xx+4*time+7;x++) {
                            for(int y=max(4*time+4,8*yy+4*time);y<=8*yy+4*time+7;y++) {
                                #pragma ivdep
                                #pragma omp simd
                                for(int z=max(4*time+4,8*zz+4*time);z<=8*zz+4*time+7;z++) {
