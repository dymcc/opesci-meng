...
void forward_tiled_time_nozz(float u[TIME][DIM][DIM][DIM], const int time_size, const int x_size, const int y_size, const int z_size){
    for (int tt=0;tt<=floord(time_size-2,8);tt++) {
        for (int xx=2*tt;xx<=min(floord(time_size+65,4),2*tt+18);xx++) {
            for (int yy=max(2*tt,xx-17);yy<=min(min(floord(time_size+65,4),2*tt+18),xx+17);yy++) {
                for (int time=max(max(max(1,8*tt),4*xx-67),4*yy-67);time<=min(min(min(time_size-2,8*tt+7),4*xx+2),4*yy+2);time++) {
                    int skew = 4*time;
                    #pragma omp parallel
                    {
...
void forward_tiled_time(float u[TIME][DIM][DIM][DIM], const int time_size, const int x_size, const int y_size, const int z_size){
    for (int tt=0;tt<=floord(time_size-2,8);tt++) {
        for (int xx=2*tt;xx<=min(floord(time_size+65,4),2*tt+18);xx++) {
            for (int yy=max(2*tt,xx-17);yy<=min(min(floord(time_size+65,4),2*tt+18),xx+17);yy++) {
                for (int zz=max(max(2*tt,xx-17),yy-17);zz<=min(min(min(floord(time_size+65,4),2*tt+18),xx+17),yy+17);zz++) {
                    #pragma omp parallel
                    {
                    for (int time=max(max(max(max(1,8*tt),4*xx-67),4*yy-67),4*zz-67);time<=min(min(min(min(time_size-2,8*tt+7),4*xx+2),4*yy+2),4*zz+2);time++) {
                        int skew = 4*time;
                        #pragma omp for schedule(static)
                        for (int x=max(16*xx,skew+4);x<=min(skew+271,16*xx+15);x++) {
                            for (int y=max(16*yy,skew+4);y<=min(skew+271,16*yy+15);y++) {
                                #pragma ivdep
                                #pragma omp simd
                                for (int z=max(16*zz,skew+4);z<=min(skew+271,16*zz+15);z++) {
                                    stencil(u, time, x-skew, y-skew, z-skew);
                                }
                            }
                        }
...
int main(){
    ...
    forward_tiled_time_nozz(v_grid, TIME, DIM, DIM, DIM);
