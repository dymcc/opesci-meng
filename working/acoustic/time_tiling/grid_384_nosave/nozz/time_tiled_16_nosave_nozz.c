...
int Forward(float *restrict damp_vec, float *restrict m_vec, float *restrict rec_vec, float *restrict rec_coords_vec, float *restrict u_vec, const int d_size, const int p_rec_size, const int t_size, const int time_size, const int x_size, const int y_size, const int z_size, struct profiler *timings)
{
    ...
    for (int tt=0;tt<=floord(time_size-2,8);tt++) {
        for (int xx=2*tt;xx<=min(floord(time_size+97,4),2*tt+26);xx++) {
            for (int yy=max(2*tt,xx-25);yy<=min(min(floord(time_size+97,4),2*tt+26),xx+25);yy++) {
                for (int time=max(max(max(1,8*tt),4*xx-99),4*yy-99);time<=min(min(min(time_size-2,8*tt+7),4*xx+2),4*yy+2);time++) {
                        int skew = 4*time;
                        int t0 = (time) % 8;
                        int t1 = (time + 1) % 8;
                        int t2 = (time - 1) % 8;
                        #pragma omp parallel
                        {
                            ...
                            for (int x=max(16*xx,4*time+4);x<=min(4*time+399,16*xx+15);x++) {
                                for (int y=max(16*yy,4*time+4);y<=min(4*time+399,16*yy+15);y++) {
                                    #pragma ivdep
                                    #pragma omp simd
