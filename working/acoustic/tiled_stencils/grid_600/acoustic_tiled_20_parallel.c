  ...
  for (int time = 1; time < time_size - 1; time += 1)
  {
    ...
    #pragma omp parallel
    {
      ...
      for (xx=0;xx<=30;xx++) {
          for (yy=0;yy<=30;yy++) {
              for (zz=0;zz<=30;zz++) {
                  for (x=max(4*time+4,4*time+20*xx);x<=min(4*time+615,4*time+20*xx+19);x++) {
                      for (y=max(4*time+4,4*time+20*yy);y<=min(4*time+615,4*time+20*yy+19);y++) {
                        #pragma ivdep
                        #pragma omp simd aligned(damp,m,u:32)
                          for (z=max(4*time+4,4*time+20*zz);z<=min(4*time+615,4*time+20*zz+19);z++) {
