  ...
  for (int time = 1; time < time_size - 1; time += 1)
  {
    ...
    #pragma omp parallel
    {
      ...
      for (xx=0;xx<=25;xx++) {
          for (yy=0;yy<=25;yy++) {
              for (zz=0;zz<=25;zz++) {
                  for (x=max(4*time+4,4*time+24*xx);x<=min(4*time+615,4*time+24*xx+23);x++) {
                      for (y=max(4*time+4,4*time+24*yy);y<=min(4*time+615,4*time+24*yy+23);y++) {
                        #pragma ivdep
                        #pragma omp simd aligned(damp,m,u:32)
                        for (z=max(4*time+4,4*time+24*zz);z<=min(4*time+615,4*time+24*zz+23);z++) {
