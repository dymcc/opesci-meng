  ...
  for (int time = 1; time < time_size - 1; time += 1)
  {
    ...
    #pragma omp parallel
    {
      ...
      for (xx=0;xx<=26;xx++) {
          for (yy=0;yy<=26;yy++) {
              for (zz=0;zz<=26;zz++) {
                  for (x=max(skew+4,skew+20*xx);x<=min(skew+527,skew+20*xx+19);x++) {
                      for (y=max(skew+4,skew+20*yy);y<=min(skew+527,skew+20*yy+19);y++) {
                          #pragma ivdep
                          #pragma omp simd aligned(damp,m,u:32)
                          for (z=max(skew+4,skew+20*zz);z<=min(skew+527,skew+20*zz+19);z++) {
