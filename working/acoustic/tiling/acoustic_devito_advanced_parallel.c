...
int Forward(float *restrict damp_vec, float *restrict m_vec, float *restrict rec_vec, float *restrict rec_coords_vec, float *restrict src_vec, float *restrict src_coords_vec, float *restrict u_vec, const int d_size, const int t_size, const int time_size, const int x_size, const int y_size, const int z_size, const int x_block_size, const int y_block_size, struct profiler *timings)
{
  ...
  printf("%d %d", x_block_size, y_block_size);
  for (int time = 1; time < time_size - 1; time += 1)
  {
    ...
    #pragma omp parallel
    {
      ...
      _MM_SET_FLUSH_ZERO_MODE(_MM_FLUSH_ZERO_ON);
      #pragma omp for schedule(static)
      for (int x = x_size - (x_size - 8)%(x_block_size) - 4; x < x_size - 4; x += 1)
      {
        for (int y = 4; y < y_size - (y_size - 8)%(y_block_size) - 4; y += 1)
        {
          #pragma noinline
          f_1_0(damp_vec,x_size,x,y_size,y,z_size,m_vec,u_vec,t_size,t0,t1,t2);
        }
    ...
    #pragma omp parallel
    {
      ...
      _MM_SET_FLUSH_ZERO_MODE(_MM_FLUSH_ZERO_ON);
      #pragma omp for schedule(static)
      for (int x = 4; x < x_size - (x_size - 8)%(x_block_size) - 4; x += 1)
      {
        for (int y = y_size - (y_size - 8)%(y_block_size) - 4; y < y_size - 4; y += 1)
        {
          #pragma noinline
          f_1_0(damp_vec,x_size,x,y_size,y,z_size,m_vec,u_vec,t_size,t0,t1,t2);
        }
