...
int Forward(float *restrict damp_vec, float *restrict m_vec, float *restrict rec_vec, float *restrict rec_coords_vec, float *restrict u_vec, const int d_size, const int p_rec_size, const int t_size, const int time_size, const int x_size, const int y_size, const int z_size, struct profiler *timings)
{
    ...
    for (int tt=0;tt<=floord(time_size-2,8);tt++) {
        for (int xx=max(1,4*tt);xx<=min(floord(time_size+97,2),4*tt+53);xx++) {
            for (int yy=max(max(1,4*tt),xx-49);yy<=min(min(floord(time_size+97,2),4*tt+53),xx+49);yy++) {
                for (int time=max(max(max(1,8*tt),2*xx-99),2*yy-99);time<=min(min(min(2*xx,2*yy),time_size-2),8*tt+7);time++) {
                        int skew = 4*time;
                        int t0 = (time) % 8;
                        int t1 = (time + 1) % 8;
                        int t2 = (time - 1) % 8;
                        #pragma omp parallel
                        {
                            ...
                            for (int x=max(8*xx,4*time+4);x<=min(4*time+399,8*xx+7);x++) {
                                for (int y=max(8*yy,4*time+4);y<=min(4*time+399,8*yy+7);y++) {
                                    #pragma ivdep
                                    #pragma omp simd
