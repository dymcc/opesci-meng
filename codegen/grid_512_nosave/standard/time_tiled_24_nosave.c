...
int Forward(float *restrict damp_vec, float *restrict m_vec, float *restrict rec_vec, float *restrict rec_coords_vec, float *restrict u_vec, const int d_size, const int p_rec_size, const int t_size, const int time_size, const int x_size, const int y_size, const int z_size, struct profiler *timings)
{
    ...
    for (int tt=0;tt<=floord(time_size-2,8);tt++) {
        for (int xx=ceild(4*tt-2,3);xx<=min(floord(4*tt+69,3),floord(time_size+129,6));xx++) {
            for (int yy=max(ceild(4*tt-2,3),xx-22);yy<=min(min(floord(4*tt+69,3),floord(time_size+129,6)),xx+22);yy++) {
                for (int zz=max(max(ceild(4*tt-2,3),xx-22),yy-22);zz<=min(min(min(floord(4*tt+69,3),floord(time_size+129,6)),xx+22),yy+22);zz++) {
                    for (int time=max(max(max(max(1,8*tt),6*xx-131),6*yy-131),6*zz-131);time<=min(min(min(min(time_size-2,8*tt+7),6*xx+4),6*yy+4),6*zz+4);time++) {
                        int skew = 4*time;
                        int t0 = (time) % 8;
                        int t1 = (time + 1) % 8;
                        int t2 = (time - 1) % 8;
                        #pragma omp parallel
                        {
                            ...
                            for (int x=max(24*xx,4*time+4);x<=min(4*time+527,24*xx+23);x++) {
                                for (int y=max(24*yy,4*time+4);y<=min(4*time+527,24*yy+23);y++) {
                                    #pragma ivdep
                                    #pragma omp simd
                                    for (int z=max(24*zz,4*time+4);z<=min(4*time+527,24*zz+23);z++) {
